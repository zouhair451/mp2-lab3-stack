...
{
	TStack prior(MaxLen);
	int j = 0; // счетчик для PostfixForm[MaxLen]
	int tmp[MaxLen];
	...
	// +(43) -(45) - 2
	// *(42) /(47) - 3
	// у нас тут два случая: когда число или когда операция
	int pr[48] = {-1};
	pr[40] = 0;
	pr[41] = 1;
	pr[43] = pr[45] = 2;
	...
	for (int i = 0; i<size_f ; ++i)
	{
		...
		if (Formula[i] == ')')
		{
			while (prior.TopElem() != '(')
			{
				PostfixForm[j++] = prior.Get(); //выталкиваем все операторы из стека
				PostfixForm[j++] = ' ';
			}
			prior.Get(); // выталкиваем и саму скобку
		} 
